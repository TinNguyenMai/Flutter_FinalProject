...
class _ForestOwnersPageState extends State<ForestOwnersPage> {
  ...
  Future<void> _showOwnerDialog({
    ...
    List<UserModel> firebaseOwners = [];
    if (!isEditing) {
      try {
        firebaseOwners = await UserService().getOwners();
      } catch (error) {
        debugPrint('Error fetching owners from Firebase: $error');
      }
    }
    UserModel? selectedFirebaseOwner;
                                        ...
                                        if (!isEditing)
                                          SizedBox(
                                            ...
                                            child: DropdownButtonFormField<UserModel>(
                                              value: selectedFirebaseOwner,
                                              isExpanded: true,
                                              decoration: const InputDecoration(
                                                labelText: 'Select Firebase Owner',
                                                border: OutlineInputBorder(),
                                              ),
                                              items: firebaseOwners.map((u) {
                                              ...
                                              validator: (val) {
                                                if (val == null) return 'Vui lòng chọn chủ rừng từ Firebase';
                                                return null;
                                              },
                                              onChanged: isSaving
                                                  ? null
                                                  : (val) {
                                                      if (val != null) {
                                                        dialogSetState(() {
                                                          selectedFirebaseOwner = val;
                                                          ownerCodeController.text = val.employeeId ?? '';
                                                          ownerNameController.text = val.fullName;
                            ...
                            ElevatedButton.icon(
                              onPressed: isSaving
                                  ...
                                  : () async {
                                      ...
                                      try {
                                        saved = await _service.saveOwner(
                                          owner: result,
                                      ...
                                      // Close the dialog first, then update UI
